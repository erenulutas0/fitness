...
| D19 | Antrenman geçmişi **Drift/SQL değil, seans başına bir JSON dosyası** (`sessions/`) | docs/05 stack'te Drift yazıyordu ama `drift_dev` analyzer ≥13 istiyor, `custom_lint` 0.8.1 ve `freezed` 3.2.3 bizi analyzer 8'de tutuyor — kurulamadı (10 Eylül). Ayrıca veri gerçekten küçük: yılda ~300 seans, 5 yılda ~1500 kayıt; SQL'in getirdiği sorgulama gücü bir kod üreteci ve native sqlite kütüphanesi (16 KB hizalaması yeniden doğrulanmalı) etmiyor. `SessionStore` bir arayüz: geçmiş gerçek sorgu isterse yalnızca o sınıf değişir. Kurucu onayıyla | 05 §9, session_store.dart |
| D20 | Bugün ekranındaki ana buton **"Hızlı form kontrolü"** (önceki adı "Hızlı form check") | TR-first bir ürünün en görünür çağrısı yarı İngilizceydi; docs/02'deki mağaza yorumu "Türkçe istiyoruz" doğrudan bunu işaret ediyor ve onboarding'den çıkan yeni kullanıcının okuduğu ilk buton bu. İngilizce metin değişmedi ("Quick form check"). Kurucu onayıyla, 10 Eylül | 06 §3, app_tr.arb |
